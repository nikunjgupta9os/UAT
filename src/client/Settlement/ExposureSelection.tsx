...
const ExposureSelection = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    // First filter: Only show approved data
                  ...
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => {
                      ...
                      return (
                        <th
                          key={header.id}
                          className="px-6 py-4 text-left text-sm font-semibold text-header-color uppercase tracking-wider border-b border-border sticky top-0 bg-secondary-color"

...
interface PreviewTableProps {
  ...
  onUpdateRow?: (rowIndex: number, updatedData: Record<string, string>) => void;
}
...
const PreviewTable: React.FC<PreviewTableProps> = ({
  ...
  const handleRowUpdate = useCallback(
    async (rowId: string, updatedFields: Record<string, string | number | boolean>) => {
      if (!onUpdateRow) return true;
      const rowIndex = parseInt(rowId.replace("row_", ""));
      if (isNaN(rowIndex)) return false;
      try {
        // Convert all values to string before passing to onUpdateRow
        const stringFields: Record<string, string> = {};
        Object.keys(updatedFields).forEach((key) => {
          stringFields[key] = String(updatedFields[key]);
        });
        onUpdateRow(rowIndex, stringFields);
        return true;
      } catch (error) {

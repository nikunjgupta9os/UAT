...
import NyneOSTable from "./NyneOSTable";
...
const sections: {
  ...
  editableKeys?: (keyof Transaction)[];
}[] = [
  {
    ...
    editableKeys: ["orderType", "transactionType", "counterparty", "status"],
  },
  {
    title: "Entity Hierarchy",
    fields: ["entityLevel0", "entityLevel1", "entityLevel2", "entityLevel3"],
    editableKeys: [
      "entityLevel0",
      "entityLevel1",
      "entityLevel2",
  ...
  },
  {
    ...
    editableKeys: ["spotRate", "forwardPoints", "bankMargin", "totalRate"],
  },
  {
    title: "Values",
    fields: [
    ...
    editableKeys: ["inputValue", "valueType"],
  },
  {
    title: "Dates & Settlement",
    fields: [
      "addDate",
      "settlementDate",
      "maturityDate",
...
const nonSortingColumns = ["expand", "action", "select"]; // Add this line
...
const TransactionTable: React.FC = () => {
      ...
      try {
        ...
        if (userTabs) {
          ...
          setColumnVisibility((prev) => ({
            ...prev,
            action: userTabs.pendingForward.showDeleteButton || false,
          }));
        }
      } catch (error) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over) return;
    // Prevent dragging to or from non-draggable columns
    if (
  ...
  const table = useReactTable({
    ...
    enableMultiSort: true, // <-- This is all you need!
  });
  console.log("COlumn", table.getAllLeafColumns());
  ...
  const handleFieldChange = (
    ...
    const updatedValues = { ...editValues, [field]: newValue };

    if (
      ["spotRate", "forwardPoints", "bankMargin", "orderType"].includes(
        field as string
      )
    ) {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <NyneOSTable<Transaction>
        ...
        onChange={handleFieldChange} // <-- Use onChange, not handleFieldChange
